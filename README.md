...
*   **OpenSearch**: поисковый и аналитический движок для хранения и индексации данных.
*   **OpenSearch Dashboards**: веб-интерфейс для визуализации и анализа данных из OpenSearch.
*   **Logstash**: серверный конвейер обработки данных для приема, преобразования и отправки данных в OpenSearch.
...
*   **opensearch-server**: доступен на портах `9200` (HTTP REST API) и `9300` (Transport).
*   **opensearch-dashboards**: доступен на порту `5601`.
*   **logstash**: настроен на монтирование папок `pipeline` и `data` для гибкой конфигурации.

### Logstash Pipeline

Пайплайн в `logstash/pipeline/logstash.conf` настроен на:
*   **Input**: Чтение из файла `access.log` в смонтированной директории `data`.
*   **Filter**: Парсинг логов с помощью Grok-паттерна для извлечения полей (client, method, status, timestamp и т.д.).
*   **Output**: Отправка структурированных данных в OpenSearch для индексации в индекс `access-logs`.
...
На первом этапе для знакомства с системой был произведён запуск контейнера OpenSearch в ручном режиме с использованием команды docker run:
...
*	Metric – общее количество событий (счётчик).
*	Pie chart – распределение HTTP Status Codes (200, 300, 400, 500).
*	Vertical bar chart – количество запросов по часам.
*	Line chart – динамика запросов во времени.
*	Data table – топ-10 клиентов по числу запросов.
*	Heatmap – нагрузка на сервис по дням недели и часам.

Все визуализации были объединены в единый дашборд, обеспечивающий комплексный мониторинг состояния веб-сервиса.

В начале я создал Index Pattern:
<div align="center">
  <img src="screens/indexes.png" alt="Создание Index Pattern" width="600">
  <p><em>Создание Index Pattern</em></p>
</div>

После этого были созданы следующие визуализации:
<div align="center">
  <img src="screens/visualizations.png" alt="Все визуализации" width="600">
  <p><em>Все визуализации</em></p>
</div>

В результаты получилось следующее: 
...
## 6. Вывод
В ходе работы была развернута система для сбора, обработки и анализа логов на базе **OpenSearch**, **Logstash** и **OpenSearch Dashboards**.
Этапы реализации:
...
## Как запустить у себя: инструкция для MacOS
...
4.  **Откройте OpenSearch Dashboards**:
    Перейдите по адресу: [http://localhost:5601](http://localhost:5601)
...
*   **Просмотр логов**:
    ```bash
    docker-compose logs [service-name]  # (opensearch-server, logstash)
    ```

*   **Удаление тома с данными** (предупреждение: это удалит все индексированные данные):
    ```bash
